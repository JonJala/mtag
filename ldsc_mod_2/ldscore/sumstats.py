...
def estimate_h2(args, log):
    ...
    if args.no_intercept:
        args.intercept_h2 = 1
    if user_frames:
        h2_frame = user_frames[0].copy()
        M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols = _read_ld_sumstats(args, log, args.h2, sumstats=h2_frame)
    else:
        M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols = _read_ld_sumstats(
        args, log, args.h2, alleles=True)
