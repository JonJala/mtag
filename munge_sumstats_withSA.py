...
#  from ldscore import sumstats_withSA
...
def munge_GWASinput(args, p=True):
    __version__ = '1.0.0'
    MASTHEAD = "*********************************************************************\n"
    ...
    VALID_SNPS_SA= {x for x in map(lambda y: ''.join(y), it.product(BASES, BASES))
                  if x[0] != x[1] } # includes strand-ambiguous SNPs

    MATCH_ALLELES_SA = {x for x in map(lambda y: ''.join(y), it.product(VALID_SNPS_SA, VALID_SNPS_SA))
    ...
    def filter_alleles(a):
        ...
        filter_flag = a.isin(VALID_SNPS_SA)
    ...
    def allele_merge(dat, alleles):
        ...
        match = a1234.apply(lambda y: y in MATCH_ALLELES_SA)
